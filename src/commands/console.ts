...
async function buildConsolePrompt(
  ...
  return [
    ...
    '  -> In [next], remind the user to review it with `author memory deltas show <name>` and manually integrate adopted entries into memory/canon.md or the relevant memory/*.yaml.',
    '  -> DO NOT suggest commands like `author memory apply-latest` or `author memory commit` - those do not exist.',
    '  -> The author manually curates memory; AuthorOS does not auto-merge.',
    '',
